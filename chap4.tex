...
$\mathbf{a}=\{a_0,a_1,a_2,...,a_N\}$ or scalar values and
a binary operator $\bigotimes$, and returns a single scalar result.
...
each layer of the balanced tree can be evaluated simultaneously and the
...
The memory requirement is one reason why the serial is chosen at
all in cases when no parallelism is available.
...
is a problem when the operator $\bigotimes$ is not commutative,
as is the case with computer implementations of floating-point operations.
