...
D. A. Ibanez, E. S. Seol, C. W. Smith, and M. S. Shephard,
``Pumi: Parallel unstructured mesh infrastructure,"
\textit{ACM Trans. Math. Softw.},
vol. 42, no. 3, pp. 17:1–17:28, May 2016.
}
\blfootnote{
...
are fed as files to a standalone mesh adaptation program, which
outputs a new mesh file back to the original simulation program
to run all over again.
The trade-off with coupling is that a loosely coupled implementation
requires few modifications (sometimes none) to the simulation program,
but the overhead of file I/O and switching programs (which may have
different parallel efficiencies) may preclude some of the more
...
This chapter covers multiple such integrations and aspects unique
to each of the simulation programs.
